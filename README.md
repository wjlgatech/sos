...
- **`/ce-plan` → `/ce-work` engine** (optional, either gear) — when a task wants rigor and durability over speed: hand step 0–2 research to `/ce-plan` for guardrails (stable **U-IDs**, per-unit test scenarios, scope, risks — no pre-written code), then `/ce-work` executes against them (idempotent, worktree-isolated, gated to a reviewed PR), and `/ce-compound` captures the learning. Skip it for small or non-software work — that's what inline is for.

📖 **Deep dive:** [`docs/goal-10x.md`](docs/goal-10x.md) — what it is (and isn't), when to use it (and when not), correct-vs-wrong usage with examples, the first-principles mechanism, and how to install it across projects **and** computers.
